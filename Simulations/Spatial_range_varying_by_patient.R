setwd("~/Google Drive/mpMRI/17.0828/")
library(polspline)
library(parallel)
library(spatstat)
library(ROCR)
...
index=1:46
n.cores=100
load("~/Google Drive/newcasename.RData")
...
load("~/Google Drive/mpMRI/17.0828/server_fillrecenter.RData")
fillin=fillrecenter
rm(fillrecenter)
for (i in 1:46)
{
...
##### get index:
  ...
  fillnew=mclapply(1:44,FUN=function(x){simulate_fillnew(x)},mc.cores=4)
...
#### Msmooth:
    ...
    save(auc_mbase,tp_mbase,auc_mregion,tp_mregion,auc_mbase1,tp_mbase1,auc_mregion1,tp_mregion1,
         ...
         file=paste("simulation_paper1/simu_oversmoothing_sigmasq_",sigmasq,"_nu",nu,".RData",sep="")) 
}
}
